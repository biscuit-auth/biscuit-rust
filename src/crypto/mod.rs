...
impl Token {
    ...
    pub fn append(&self, keypair: &KeyPair, message: &[u8]) -> Self {
        let signature = self
            .signature
            .sign(&self.keys, &self.messages, keypair, message);
...
impl TokenSignature {
    ...
    pub fn sign<M: Deref<Target = [u8]>>(
        ...
        let hashes = messages
            ...
            .map(|(m, pk)| ECVRF_hash_to_curve(*pk, m))
            .collect::<Vec<_>>();
        ...
        let v = self
            ...
            .iter()
            .zip(self.c.iter())
            .fold(self.w, |acc, (gamma, c)| (c.neg() * gamma) + acc)
            + (self.s * hashes_sum)
            + (k * h);
